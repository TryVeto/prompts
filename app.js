...
const keyMap={'1':'H','2':'04','3':'05','4':'07',p:'P'};
...
document.addEventListener('keydown',e=>{if(e.defaultPrevented||e.isComposing||composing||e.repeat)return;const mod=e.metaKey||e.ctrlKey;if(openId){if(!e.metaKey&&!e.ctrlKey&&!e.altKey&&e.key.toLowerCase()==='c'&&!document.querySelector('dialog[open]')&&!e.composedPath().some(x=>x instanceof Element&&(x.matches('input,textarea,select,[role=textbox],[role=searchbox],[role=combobox]')||x.isContentEditable))){e.preventDefault();copyPrompt();return}if(e.key==='Escape'){e.preventDefault();goBack()}else if(mod&&e.key==='Enter'){e.preventDefault();copyPrompt()}return}if(mod&&!e.altKey&&e.key.toLowerCase()==='k'){e.preventDefault();$('#find').focus();$('#find').select();return}if(activeInput(e))return;if(e.key==='Enter'){e.preventDefault();if(row(chosen))openReader(chosen);return}if(e.key==='ArrowDown'||e.key==='ArrowUp'){e.preventDefault();move(e.key==='ArrowDown'?1:-1);return}if(e.metaKey||e.ctrlKey||e.altKey||e.shiftKey)return;const id=keyMap[e.key.toLowerCase()];if(id&&row(id)){e.preventDefault();setChosen(id,true,false);copySelected(id)}});
...
$('#reader-copy').title='Copy prompt (C)';
$('#reader-copy').setAttribute('aria-keyshortcuts','c');
